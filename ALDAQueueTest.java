...
public class ALDAQueueTest {
	...
	@Test
	public void testDiscriminateObjectsWithIdentity() {
		ALDAQueue<String> queue = new MyALDAQueue<String>(DEFAULT_CAPACITY);
		queue.add(new String("STRING"));
		queue.add(new String("ANOTHER STRING"));
		queue.add(new String("STRING"));
		queue.add(new String("ANOTHER STRING"));
		queue.add(new String("STRING"));

		assertEquals(3, queue.discriminate("STRING"));
		testQueueProperties(queue, false, false, 5, DEFAULT_CAPACITY, DEFAULT_CAPACITY - 5,
				"[ANOTHER STRING, ANOTHER STRING, STRING, STRING, STRING]");

		// Remove the non-discriminated elements
		queue.remove();
		queue.remove();

		String first = queue.remove();
		assertNotSame(first, queue.remove(),
				"The queue contains unique items that now seems to have been replaced with aliases to the same object");
		assertNotSame(first, queue.remove(),
				"The queue contains unique items that now seems to have been replaced with aliases to the same object");
	}
